...
JS was by Brendan Eich from Netscape (early browser) as an early browser-based scripting language.
...
- **`this`, Binding, Clojures** 
  - determining `this` in a function context 
  - determining scope during function invokation
...
Functions are important for many reasons we are already familiar with from. They allow you to:
...
1. Compare output with partner
1. Analyze if you aggree and describe the control flow
...
A **closure is binding of a function with its enclosing scope**.
...
**Clojures are how Node keeps setup data (config, a name, a db connection) across later requests.**
...
**Clojures are how React handlers keep the props they were created with.**
...
**💡 See Node.js examples in:** `demos/06_node_npm.js`
