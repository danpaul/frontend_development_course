...
For today, pull updates from the repos:
...
Switch to **week_2** branch on the code repo
...
```javascript
const welcome = (name) => `Welcome to Wonderland, ${name}!`;
...
console.log(greet((n) => `We're all mad here, ${n}!`));
...
var alice = new Character { Name = "Alice" };
alice.Greet();           // Alice
...
function queenOfHearts(name) {
  const decree = name === "Alice" ? "Off with her head!" : "Off with his head!";
...
function SentenceButton({ name }) {
  ...
  return <button onClick={handleClick}>Sentence {name}</button>;
}
```
...
Open `/04_court/`, run `node server.js`, open [http://localhost:3000](http://localhost:3000). **Don't use AI.**
...
Unlike many other popular programming languages (C#, Java, PHP), **async functionality is built into JavaScript and is an essential part of most JS codebases**.
...
Open `/05_white_rabbit.js`. **Don't run it or use AI.**
...
1. First log runs immediately — `getJson` _starts_ the request and returns
2. The "Meanwhile" log runs while the network is still in flight
3. When the reader arrives, that callback runs and starts the titles request
4. When the titles arrive, the nested callback runs

The second request lives _inside_ the first callback, so it cannot start until the first response is in. That nesting is the start of callback hell.
...
```javascript
askCheshireCat()
  .then((cat) => findMadHatter(cat.direction))
  .then((hatter) => followWhiteRabbit(hatter.hint))
  .then((rabbit) => enterQueensCourt(rabbit.hole))
  .then((court) => console.log("Alice arrives at:", court.name))
  .catch((error) => console.error("Lost in Wonderland:", error));
```
...
Open `/05_white_rabbit.js` and `/06_white_rabbit_promise.js`.
...
```javascript
Promise.all([askCheshireCat(), visitMadHatter(), findWhiteRabbit()])
  .then(([cat, hatter, rabbit]) => {
    console.log(cat.grin, hatter.riddle, rabbit.excuse);
  })
  .catch((error) => console.error("Lost in Wonderland:", error));
```
...
`async`/`await` is syntax on top of promises. Same event loop, same three states — code that reads top to bottom.

- **`async`** — marks a function. It always returns a Promise.
- **`await`** — pauses _that function_ until the Promise settles, then gives you the value. The rest of the program keeps running.

```javascript
async function enterWonderland() {
  const rabbit = await followWhiteRabbit();
  const court = await enterQueensCourt(rabbit.hole);
  console.log("Alice arrives at:", court.name);
}

enterWonderland();
console.log("Alice: Meanwhile I'll look around.");
```

`await` is only legal inside an `async` function (or at the top level of a module).
...
Same Cheshire Cat chain. Each `await` is one `.then()`. Sequential awaits wait **in order** — like a promise chain.

```javascript
async function findTheCourt() {
  const cat = await askCheshireCat();
  const hatter = await findMadHatter(cat.direction);
  const rabbit = await followWhiteRabbit(hatter.hint);
  const court = await enterQueensCourt(rabbit.hole);
  console.log("Alice arrives at:", court.name);
}

findTheCourt().catch((error) => console.error("Lost in Wonderland:", error));
```
...
`await` throws when a Promise rejects, so you can use ordinary `try` / `catch` / `finally`.
...
async function findTheCourt() {
  try {
    const cat = await askCheshireCat();
    const hatter = await findMadHatter(cat.direction);
    console.log("Alice arrives at:", hatter.table);
  } catch (error) {
    console.error("Lost in Wonderland:", error);
  } finally {
    console.log("The court is always watching.");
  }
}
```

- `try` — success path (`.then()`)
- `catch` — any rejection in the `try` (`.catch()`)
...
function Welcome(props) {
  return <h1>Welcome to Wonderland, {props.name}!</h1>;
}

// with destructuring
function Welcome({ name }) {
  return <h1>Welcome to Wonderland, {name}!</h1>;
}
...
```javascript
const seats = ["Mad Hatter", "March Hare"];
const [hatter, hare] = seats;

console.log(`The first seat belongs to the ${hatter}`);
```
...
```javascript
const guest = { place: "tea party", isLate: false };

// spread guest properties into the Hatter
const hatter = { ...guest, name: "Mad Hatter" };

console.log(hatter.place); // ~> tea party

let guests = [hatter];

// spread guest elements into the guests array and add a new guest
guests = [...guests, { ...guest, name: "March Hare" }];
```
...
// spread guest and override isLate when seating the White Rabbit
const rabbit = { ...guest, isLate: true, name: "White Rabbit" };

console.log(rabbit.isLate); // ~> true
```
...
```javascript
const seatGuest = ({ place = "tea party", isLate = false, name } = {}) => {
  console.log(`${name} is at the ${place} and is late: ${isLate}`);
};
seatGuest({ name: "Alice" });
// ~> Alice is at the tea party and is late: false
```
...
**Spread** copies properties _into_ a new value. **Rest** collects remaining properties _out_ of one.
...
```javascript
// index.js
const grin = require("./cheshire");
grin(); // ~> We're all mad here.
```
...
// named export
export const defaultBlend = "earl grey";

// named export (function)
export const pourTea = ({ blend, guest }) => {
  console.log(`Pouring ${blend} for ${guest}`);
};

// default export, class
export default class TeaParty {
  constructor({ blend = defaultBlend, guest } = {}) {
    this.blend = blend;
    this.guest = guest;
  }
}
...
The `default export` (the `TeaParty` class) does not need to be destructured but the non-default exports do.

```javascript
// import named exports
import { defaultBlend, pourTea } from "./tea.js";
```

```javascript
// import default export
import TeaParty from "./tea.js";
```

```javascript
// import both
import TeaParty, { defaultBlend, pourTea } from "./tea.js";
```
...
![bg contain right:38%](./assets/alice_keyhole.jpg)
...
- **Syntax**: chain `map`/`filter`/`reduce`; destructure; spread _into_ and rest _out of_. Spread copies the top level only.
...
**Quiz:** `/08_js_quiz/` — open `index.html` in a browser (20 questions, score at the end).
